...
void loop()
{
  ...
  {
    String input = Serial.readString();                 //getting the
input - "A1" or "A2" or "C7" or "B3"
    char targetLocation = input.charAt(0);              // Parsing the
input for the letter - 'A' or 'B' or 'C'
    int targetIndex = String(input.charAt(1)).toInt();  // Parsing the
input for the number - 0, 1, 2, 3, 4, 5, 6, 7
    //Serial.print(currentLocation == 'A');
    //Serial.print(targetIndex);
    //Serial.println(numOfPulses);
    if((currentLocation == 'A' || currentLocation == 'B') &&
(targetLocation == 'A' || targetLocation == 'B'))
    {
    ...
    else if((currentLocation == 'A' || currentLocation == 'B') &&
(targetLocation == 'C'))
    {
    ...
    else if((currentLocation == 'C') && (targetLocation == 'A' ||
targetLocation == 'B'))
    {
    ...
    currentLocation = targetLocation; //Current position is now the
old target position
...
//    delay(1000); // One second delay
//  } else if (x==2) {
//    digitalWrite(dirPin,LOW); // Enables the motor to move in a
particular direction
//    // Makes 200 pulses for making one full cycle rotation
//    for(int y = 0; y < 600; y++) {
//      digitalWrite(stepPin,HIGH);
